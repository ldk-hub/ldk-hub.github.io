...
## 날짜테이블 샘플링 테이블 생성 기준일자 테이블

CREATE TABLE MYSTKDB.BASEDATE
( BAS_DT DATE NOT NULL
,BAS_YMD VARCHAR2(8) NOT NULL
,BAS_DT_NO INT NOT NULL
,BAS_WKD VARCHAR2(10) NOT NULL
,BAS_YM VARCHAR2(6) NOT NULL
,BAS_YY VARCHAR2(4) NOT NULL
,BAS_YQ VARCHAR2(5) NOT NULL
);
CREATE UNIQUE INDEX MYSTKDB.PK_BASEDATE ON MYSTKDB.BASEDATE(BAS_DT);
ALTER TABLE MYSTKDB.BASEDATE
ADD CONSTRAINT PK_BASEDATE PRIMARY KEY(BAS_DT)
USING INDEX MYSTKDB.PK_BASEDATE;
COMMENT ON TABLE MYSTKDB.BASEDATE IS '기준일자';
COMMENT ON COLUMN MYSTKDB.BASEDATE.BAS_YMD IS '기준일자_YMD';
COMMENT ON COLUMN MYSTKDB.BASEDATE.BAS_DT_NO IS '기준일자순번';
COMMENT ON COLUMN MYSTKDB.BASEDATE.BAS_WKD IS '기준요일'; COMMENT ON COLUMN 
MYSTKDB.BASEDATE.BAS_YM IS '기준월';
COMMENT ON COLUMN MYSTKDB.BASEDATE.BAS_YY IS '기준년';
COMMENT ON COLUMN MYSTKDB.BASEDATE.BAS_YQ IS '기준분기'


```
SELECT TO_DATE('20190101','YYYYMMDD') + (RNO-1) BAS_DT
FROM (
SELECT ROWNUM RNO
FROM DUAL
